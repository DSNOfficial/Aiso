...
const getList = async (req, res) => {
    try {
        ...
        let sql = "SELECT * FROM tbservicepartcontact WHERE 1=1";
        let sqlWhere = "";
        let param = {};
...
const TrainImage = async (req, res) => {
    try {
        ...
        var sql = "SELECT * FROM tbservicepartcontact_image WHERE tbservicepartcontact_id =:tbservicepartcontact_id";

        var [list] = await db.query(sql, {
            tbservicepartcontact_id: req.params.tbservicepartcontact_id,
        });
        res.json({
            list,
        });
        ...
        logError("tbservicepartcontact.TrainImage", err, res);
    }
};
...
const getOne = async (req, res) => {
    try {
        ...
        const sqlTraining = "SELECT * FROM tbservicepartcontact WHERE id = :id";
        ...
        const sqlImages = "SELECT image FROM tbservicepartcontact_image WHERE tbservicepartcontact_id = :id";
        ...
        logError("tbservicepartcontact.getOne", err, res);
    }
};
...
const create = async (req, res) => {
    try {
        const sql = `
            INSERT INTO tbservicepartcontact (title,description,link,status,image)
            VALUES (:title, :description,:link, :status, :image)
        `;
        ...
        if (req.files && req.files?.upload_image_optional ) {
            ...
            var slqTrainImage = "INSERT INTO tbservicepartcontact_image (tbservicepartcontact_id,image) VALUES :data";
        ...
        logError("tbservicepartcontact.create", err, res);
    }
};
...
const update = async (req, res) => {
    try {
        ...
        var sql = "UPDATE tbservicepartcontact SET  title=:title, link=:link, image=:image, description=:description, status=:status WHERE id = :id";
        ...
        if (req.files && req.files?.upload_image_optional) {
            ...
            var slqTrainImage = "INSERT INTO servicepartcontact_image (tbservicepartcontact_id,image) VALUES :data";
            var [dataImage] = await db.query(slqTrainImage, {
                data: ParaImageTrain,
            });
        ...
        if (req.body.image_optional && req.body.image_optional.length > 0) {
            ...
            req.body.image_optional.map(async (item, index) => {
                ...
                let [data] = await db.query("DELETE FROM servicepartcontact_image WHERE image =:image",
                    { image: item }
                );
        ...
        logError("tbservicepartcontact.update", err, res);
    }
};
...
const remove = async (req, res) => {
    try {
        ...
        const [dataInfo] = await db.query("SELECT * FROM tbservicepartcontact WHERE id=:id", param);

        if (dataInfo.length > 0) {
            // Delete associated optional images first
            const [imageRows] = await db.query("SELECT image FROM tbservicepartcontact_image WHERE tbservicepartcontact_id = :id", param);
            for (const img of imageRows) {
                await removeFile(img.image); // remove each optional image
            }

            // Then delete the records from tbtraining_image
            await db.query("DELETE FROM tbservicepartcontact_image WHERE tbservicepartcontact_id = :id", param);

            // Delete main training record
            const sql = "DELETE FROM tbservicepartcontact WHERE id = :id";
