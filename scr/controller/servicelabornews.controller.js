...
const getList = async (req, res) => {
    try {
        ...
        let sql = "SELECT * FROM tbservicelabornews WHERE 1=1";
        let sqlWhere = "";
        let param = {};
        ...
        logError("tbservicelabornews.getList", err, res);
    }
}
...
const TrainImage = async (req, res) => {
    try {
        ...
        var sql = "SELECT * FROM tbservicelabornews_image WHERE tbservicelabornews_id =:tbservicelabornews_id";

        var [list] = await db.query(sql, {
            tbservicelabornews_id: req.params.tbservicelabornews_id,
        });
        res.json({
            list,
        });
        ...
        logError("tbservicelabornews.TrainImage", err, res);
    }
};
...
const getOne = async (req, res) => {
    try {
        ...
        const sqlTraining = "SELECT * FROM tbservicelabornews WHERE id = :id";
        ...
        const sqlImages = "SELECT image FROM tbservicelabornews_image WHERE tbservicelabornews_id = :id";
        ...
        logError("tbservicelabornews.getOne", err, res);
    }
};
...
const create = async (req, res) => {
    try {
        ...
        const sql = `
            INSERT INTO tbservicelabornews (title,description,status,image)
        ...
        if (req.files && req.files?.upload_image_optional ) {
            ...
            var slqTrainImage = "INSERT INTO tbservicelabornews_image (tbservicelabornews_id,image) VALUES :data";
        ...
        logError("tbservicelabornews.create", err, res);
    }
};
...
const update = async (req, res) => {
    try {
        ...
        var sql = "UPDATE tbservicelabornews SET  title=:title ,image=:image,description=:description,status=:status WHERE id = :id";
        ...
        if (req.body.image_optional && req.body.image_optional.length > 0) {
            ...
            req.body.image_optional.map(async (item, index) => {
                ...
                let [data] = await db.query("DELETE FROM tbservicelabornews_image WHERE image =:image",
                    { image: item }
                );
        ...
        logError("tbservicelabornews.update", err, res);
    }
};
...
const remove = async (req, res) => {
    try {
        ...
        const [dataInfo] = await db.query("SELECT * FROM tbservicelabornews WHERE id=:id", param);

        if (dataInfo.length > 0) {
            // Delete associated optional images first
            const [imageRows] = await db.query("SELECT image FROM tbservicelabornews_image WHERE tbservicelabornews_id = :id", param);
            for (const img of imageRows) {
                await removeFile(img.image); // remove each optional image
            }

            // Then delete the records from tbtraining_image
            await db.query("DELETE FROM tbservicelabornews_image WHERE tbservicelabornews_id = :id", param);

            // Delete main training record
            const sql = "DELETE FROM tbservicelabornews WHERE id = :id";
        ...
        logError("tbservicelabornews.remove", err, res);
    }
};
